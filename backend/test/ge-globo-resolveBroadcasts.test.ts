...
describe("resolveBroadcasts", () => {
  it("drops globoplay but keeps the real channels — the actual Náutico x Athletic case: sportv+Premiere+globoplay all present, article confirms only Premiere+SporTV", () => {
    const result = resolveBroadcasts([source("sportv"), source("Premiere"), source("globoplay")]);
