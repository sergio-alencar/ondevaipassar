...
function buildMatch(overrides: Partial<MatchView> = {}): MatchView {
  return {
    ...
    round: 20,
    status: "scheduled",
    broadcasts: [{ channelId: "globo", displayName: "Globo", kind: "tv" as const, free: true, url: "https://globo.com", logoUrl: "", regionalCaveat: true }],
    ...overrides,
  };
}
    ...
    const png = await renderMatchImage(
      buildMatch({
        broadcasts: [
          ...
          { channelId: "sportv", displayName: "SporTV", kind: "tv" as const, free: false, url: "", logoUrl: "", regionalCaveat: false },
          { channelId: "tntsports", displayName: "TNT Sports", kind: "youtube" as const, free: true, url: "", logoUrl: "", regionalCaveat: false },
        ],
      }),
    );
    expect(isPng(png)).toBe(true);
