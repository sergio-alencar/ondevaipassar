...
import { fetchArticleCandidate } from "../sources/futebol-interior/article.js";
import { fetchCandidateArticleUrls } from "../sources/futebol-interior/client.js";
...
const SOURCE_ID = "futebol-interior";
const CONCURRENCY = 5;
...
async function resolveArticle(url: string, allMatches: MatchCandidate[]): Promise<ArticleResult> {
  ...
  if (!article) return null; // not a preview article (recap, transfer news, etc.) — doesn't count against run health
...
/**
 * Supplementary source for Série C, additive-only, same shape as
 * ondeAssistirEnrichment.ts: ge.globo's own Série C round-hub adapter
 * (ge-globo-round) only gives fixtures, never broadcasts, and most Série C
 * teams don't have a discoverable ge.globo agenda page of their own either
 * — Futebol Interior (futebolinterior.com.br) is a dedicated Série C
 * editorial source with its own "Onde assistir" field per preview article.
 * Discovery is one hub page (unlike ge.globo's per-team news feeds), since
 * Série C's own hub already lists every team's upcoming preview.
 */
export async function runFutebolInteriorEnrichment(): Promise<void> {
