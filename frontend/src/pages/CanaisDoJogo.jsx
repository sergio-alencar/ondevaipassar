...
const CanaisDoJogo = ({ canais: canaisJogo, jogo, time }) => {
      ...
      <div className="overflow-x-auto whitespace-nowrap">
        ...
        >
          {separarCanais(jogo.canais).map((canal, idx) => {
            ...
            return (
              imagemCanal && (
                <a key={`${canal}-${idx}`} href={url} target="_blank" rel="noopener noreferrer">
                  <img
                    src={imagemCanal}
                    alt={alt}
                    title={title}
                    ...
                    loading="lazy"
                    onError={(e) => {
                      console.error(`Erro ao carregar imagem do canal: ${canal}`);
  ...
  return (
    <div className="text-center">
      <p className={`text=${time?.cor || "gray-500"} uppercase font-bold text-lg max-lg:text-base`}>
