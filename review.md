...
- [ ] **Fase 3**: já rolando parcialmente de graça — como o adapter ge.globo é por time (não por campeonato), Copa do Brasil e Brasileirão Série B **já aparecem** nos dados hoje, sem código novo (confirmado ao vivo: `brasileirao-serie-a`, `brasileirao-serie-b`, `copa-do-brasil` nos jogos ingeridos). Falta: confirmar Libertadores/Sul-Americana aparecendo (times rastreados atualmente podem não estar classificados), e a decisão do Sérgio de expandir o registro de times rastreados pra ~40 clubes (Série A + B) — ver "Pendências" abaixo.
